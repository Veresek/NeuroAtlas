			...
			<>
				...
				<div className="flex items-center px-4 py-2 border-b border-gray-200/60 shrink-0">
					...
					<div className="flex-1 min-w-0">
						<p className="text-[11px] font-bold uppercase tracking-widest text-[#00aaff]">{section}</p>
						<h2 className="text-sm font-bold text-gray-800 leading-tight truncate" title={item || ""}>
							{item}
						</h2>
					</div>
					<button
				...
				<div className="flex-1 overflow-y-auto p-4 text-center">
					<p className="text-gray-500 italic">Publication details not found.</p>
				</div>
			</>
		);
	}
			...
			<div className="flex-1 overflow-y-auto p-4 space-y-4 text-sm text-gray-600">
				...
				<div className="p-3 bg-gray-50 rounded-xl border border-gray-200/50 space-y-2">
					<div className="space-y-0.5">
						<p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Authors</p>
						<p className="text-xs font-semibold text-gray-700 leading-relaxed">{paper.authors.join(", ")}</p>
					</div>
					<div className="grid grid-cols-2 gap-2 pt-1.5 border-t border-gray-200/40">
						<div>
							<p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Year</p>
							<p className="text-xs font-bold text-[#00aaff]">{paper.year}</p>
						</div>
						<div>
							<p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Journal</p>
							<p className="text-xs font-medium text-gray-600 truncate" title={`${paper.journal} ${paper.volume || ""}`}>
				...
				<div>
					...
					<p className="leading-relaxed text-xs text-gray-600 bg-white border border-gray-100 rounded-xl p-3 shadow-sm">
						{paper.abstract}
					</p>
				</div>
				...
				{paper.relatedBrainAreas && (
					<BrainSectionList
						...
						titleClassName="font-bold text-gray-800 mb-1.5"
						resetKey={item}
					/>
				)}
				...
				{relatedTopics.length > 0 && (
					<div>
						<h3 className="font-bold text-gray-800 mb-1.5">Related Topics</h3>
						<div className="flex flex-wrap gap-1.5">
							{relatedTopics.map((topicId, idx) => {
								...
								return (
									<span
										key={`${topicId}-${idx}`}
										className="inline-block px-2.5 py-0.5 text-xs rounded-lg font-medium border border-gray-200 bg-gray-50 text-gray-600"
									>
										{topicName}
									</span>
								);
				...
				{/* Action button: Read Full Paper */}
				<div className="pt-2">
					<a
						href={paper.doi}
						target="_blank"
						rel="noopener noreferrer"
						className="flex items-center justify-center gap-2 w-full py-2.5 px-4 rounded-xl border border-[#00aaff]/25 text-[#00aaff] hover:bg-[#00aaff]/5 hover:border-[#00aaff]/50 font-bold text-xs transition-all duration-250 cursor-pointer shadow-sm bg-white"
					>
